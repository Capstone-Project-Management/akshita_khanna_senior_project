...
#Importing data
data <- read_csv("usa_00005.csv.gz", show_col_types=FALSE)
...
# summary(ageModel)
ggplot() +
  geom_point(data = data, aes(x = AGE, y = aiExposure), alpha = 0.4) +  # actual data points
  geom_line(data = new_data, aes(x = AGE, y = predicted), color = "blue", size = 1.2) +  # model prediction
  labs(title = "AI Exposure vs. Age",
       x = "Age", y = "AI Exposure") +
  theme_minimal()
...
ggplot() +
  geom_point(data = data, aes(x = YRSUSA1, y = aiExposure), alpha = 0.4) +  # actual data points
  geom_line(data = new_data, aes(x = YRSUSA1, y = predicted), color = "blue", size = 1.2) +  # model prediction
  labs(title = "AI Exposure vs. Duration of Residency in the U.S.",
       x = "Years in the USA", y = "AI Exposure") +
  theme_minimal()

IncModel <- lm(aiExposure ~ poly(INCWAGE, 2) + as.factor(YEAR), data=data)
# summary(IncModel)
ggplot() +
  geom_point(data = data, aes(x = INCWAGE, y = aiExposure), alpha = 0.4) +  # actual data points
  geom_line(data = new_data, aes(x = INCWAGE, y = predicted), color = "blue", size = 1.2) +  # model prediction
